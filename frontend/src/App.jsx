            ...
            {agingCurves && (
              <div className="projection-section">
                ...
                {agingExpanded && (() => {
                  ...
                  const archetypes = Object.keys(agingCurves)
                  const displayArchetype = agingArchetype || (projection?.archetype && archetypes.includes(projection.archetype) ? projection.archetype : archetypes[0])
                  const rows = agingCurves[displayArchetype] || []
                  const playerAge = playerStats?.player?.age ? Math.floor(playerStats.player.age) : null

                  return (
                    <div style={{ padding: '12px 16px 16px' }}>
                      <div className="rank-pills" style={{ marginBottom: 14 }}>
                        {archetypes.map(a => (
                          <button
                            key={a}
                            className={`rank-pill${displayArchetype === a ? ' active' : ''}`}
                      ...
                      <div style={{ overflowX: 'auto' }}>
                        <table className="aging-table">
                          <thead>
                            <tr>
                              <th className="aging-th-age">Age</th>
                              <th className="aging-th-n">n</th>
                              {AGING_COLS.map(c => <th key={c.key} className="aging-th-stat">{c.label}</th>)}
                            </tr>
                          </thead>
                          <tbody>
                            {rows.map((row, i) => {
                              const prev = i > 0 ? rows[i - 1] : null
                              const isCurrent = playerAge != null && row.age === playerAge
                              return (
                                <tr key={row.age} className={isCurrent ? 'aging-row-current' : ''}>
                                  <td className="aging-td-age">{row.age}{isCurrent ? ' ←' : ''}</td>
                                  <td className="aging-td-n">{row.n}</td>
                                  {AGING_COLS.map(c => {
                                    const val  = row[c.key]
                                    const pval = prev?.[c.key]
                                    const isFg = c.key === 'fg_pct'
                                    const fmt  = isFg ? (v => v != null ? `${v.toFixed(1)}%` : '—') : (v => v != null ? v.toFixed(1) : '—')
                                    let dir = 0
                                    if (val != null && pval != null) {
                                      const better = c.invert ? val < pval : val > pval
                                      dir = better ? 1 : val === pval ? 0 : -1
                                    }
                                    return (
                                      <td key={c.key} className={`aging-td-stat ${dir > 0 ? 'aging-up' : dir < 0 ? 'aging-dn' : ''}`}>
                                        {fmt(val)}
                                      </td>
                                    )
                                  })}
                      ...
                      <p className="aging-note">Per-30 min averages · {displayArchetype} archetype · n = players in bucket</p>
                    </div>
                  )
                })()}
              </div>
