                  ...
                  {players.map(p => {
                    ...
                    return (
                      <tr key={p.slug} className={`adj-row${hasAdj ? ' adj-row-live' : ''}`}>
                        ...
                        <td className="adj-td">
                          <input type="number" step="1" min="1" max="82" className="adj-input" style={{width:'56px'}}
                            value={edits[p.slug]?.games ?? 82}
                            onChange={ev => setField(p.slug, 'games', ev.target.value)} />
                        </td>
                        <td className="adj-td">{numInput(p.slug, 'min_pg',    '66px')}</td>
                        <td className="adj-td">{numInput(p.slug, 'fg2a_pg',   '64px')}</td>
                        <td className="adj-td">{numInput(p.slug, 'fg2_pct',   '64px')}</td>
                        <td className="adj-td">{numInput(p.slug, 'fg3a_pg',   '64px')}</td>
                        <td className="adj-td">{numInput(p.slug, 'fg3_pct',   '64px')}</td>
                        ...
                        <td className="adj-td">{numInput(p.slug, 'fta_pg',    '64px')}</td>
                        <td className="adj-td">{numInput(p.slug, 'ft_pct',    '64px')}</td>
                        <td className="adj-td adj-td-rate">{numInput(p.slug, 'oreb_rate', '64px')}</td>
                        <td className="adj-td adj-td-rate">{numInput(p.slug, 'dreb_rate', '64px')}</td>
                        <td className="adj-td adj-td-rate">{numInput(p.slug, 'ast_rate',  '64px')}</td>
                        <td className="adj-td adj-td-rate">{numInput(p.slug, 'stl_rate',  '62px')}</td>
                        <td className="adj-td adj-td-rate">{numInput(p.slug, 'blk_rate',  '62px')}</td>
                        <td className="adj-td adj-td-rate">{numInput(p.slug, 'tov_rate',  '62px')}</td>
      ...
      {simResult && (() => {
        ...
        return (
          <div style={{marginTop:20}}>
            ...
            {/* Category Win Rates */}
            {(simResult.cat_beats_orig && simResult.cat_beats_new) && (() => {
              const n = simResult.total_teams || 1
              const catB = simResult.cat_beats_orig
              const catA = simResult.cat_beats_new
              const fmtPct = v => Math.round(v * 100) + '%'
              const fmtDelta = v => (v > 0 ? '+' : '') + Math.round(v * 100) + '%'
              const totalB = cats.reduce((s, c) => s + (catB[c] != null ? catB[c] / n : 0), 0)
              const totalA = cats.reduce((s, c) => s + (catA[c] != null ? catA[c] / n : 0), 0)
              const deltaTotal = totalA - totalB
              const posCls  = v => v >  0.05 ? 'ra-z-pos' : v < -0.05 ? 'ra-z-neg' : ''
              return (
                <div style={{marginTop:24}}>
                  <div className="ra-section-title">Category Win Rates</div>
                  <div className="dash-card" style={{overflowX:'auto'}}>
                    <table className="dash-table ra-table ctw-summary-table">
                      <thead>
                        <tr>
                          <th></th>
                          {cats.map(c => <th key={c} className="num">{c}</th>)}
                          <th className="num">Total wins</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr>
                          <td className="ctw-row-label">Before</td>
                          {cats.map(c => {
                            const v = catB[c] != null ? catB[c] / n : null
                            return <td key={c} className="num mono">{v != null ? fmtPct(v) : '—'}</td>
                          })}
                          <td className="num mono ctw-total-cell">{totalB.toFixed(1)}</td>
                        </tr>
                        <tr>
                          <td className="ctw-row-label">After</td>
                          {cats.map(c => {
                            const v = catA[c] != null ? catA[c] / n : null
                            return <td key={c} className="num mono">{v != null ? fmtPct(v) : '—'}</td>
                          })}
                          <td className="num mono ctw-total-cell">{totalA.toFixed(1)}</td>
                        </tr>
                        <tr className="ctw-delta-row">
                          <td className="ctw-row-label">Δ</td>
                          {cats.map(c => {
                            const b = catB[c] != null ? catB[c] / n : null
                            const a = catA[c] != null ? catA[c] / n : null
                            const d = (a != null && b != null) ? a - b : null
                            return (
                              <td key={c} className={`num mono ${posCls(d ?? 0)}`}>
                                {d != null ? fmtDelta(d) : '—'}
                              </td>
                            )
                          })}
                          <td className={`num mono ctw-total-cell ${posCls(deltaTotal)}`}>
...
function AppMain({ onLogout, onOpenAccount, onOpenLogin, token }) {
  ...
  const handleAnalyse = async () => {
    // ── With/Without mode ───────────────────────────────────────────────
    if (wwMode) {
      if (!selectedPlayer || !wwCompanion || !wwStart || !wwEnd) {
  ...
  const STAT_COLS = [
    ...
    { key: '_2pa_pg', label: '2PA/M', noZ: true, fmt: d => {
      ...
      return `${pm.toFixed(1)}/${pa.toFixed(1)}`
    }},
    { key: '_2p_pct', label: '2P%',   noZ: true, fmt: d => {
      if (d?.fga_pg == null || d?.fg3a_pg == null || d?.fg_pct == null || d?.fg3m == null) return '—'
      const pa = d.fga_pg - d.fg3a_pg
      const pm = d.fga_pg * d.fg_pct / 100 - d.fg3m
        ...
        {selectedPlayer && playerStats && (
          <div className="player-profile">
                ...
                <div className="controls-inner">
                  ...
                  <div className="ctrl-group">
                    <div className="hist-mode-toggle">
                      <button className={`hist-mode-btn${!wwMode ? ' active' : ''}`} onClick={() => { setWwMode(false); setResult(null); setZResult(null); setZBreakdown(null) }}>Period vs Period</button>
                      <button className={`hist-mode-btn${wwMode  ? ' active' : ''}`} onClick={() => { setWwMode(true);  setResult(null); setZResult(null); setZBreakdown(null) }}>With / Without</button>
                    </div>
                  </div>

                  {/* Stat selector — always shown */}
                  <div className="ctrl-group">
                  ...
                  {!wwMode ? <>
                    ...
                    <div className="ctrl-group ctrl-period">
                      <span className="ctrl-label">Date window</span>
                      <div className="date-pair">
                        <input className="ctrl-input date-input" type="date" value={wwStart} onChange={e => setWwStart(e.target.value)} />
                        <span className="date-sep">–</span>
                  ...
                  <div className="driver-results">
                    <div className="metrics-row">
                      <div className="metric-card">
                        <span className="metric-label">{zResult.mode === 'with_without' ? zResult.period_a.start : 'Baseline Z'}</span>
                        <span className="metric-value">{zResult.period_a.z_total.toFixed(2)}</span>
                        <span className="metric-sub">{zResult.mode === 'with_without' ? zResult.period_a.end : `${zResult.period_a.start} – ${zResult.period_a.end}`}</span>
                      </div>
                      <div className="metric-card">
                      ...
                      <div className="metric-card">
                        <span className="metric-label">{zResult.mode === 'with_without' ? zResult.period_b.start : 'Comparison Z'}</span>
              ...
              <div className="projection-section">
                ...
                {schedExpanded && (schedProj.games.length === 0
                  ...
                  const chartOptions = {
                    ...
                    scales: {
                      x: { ticks: { color: isDark() ? '#ffffff' : '#1a1d2e', font: { size: 11 } }, grid: { color: isDark() ? '#1e2740' : 'rgba(0,0,0,0.07)' } },
                      y: { ticks: { color: isDark() ? '#ffffff' : '#1a1d2e', font: { size: 11 } }, grid: { color: isDark() ? '#1e2740' : 'rgba(0,0,0,0.07)' }, beginAtZero: true },
                    },
                  }
            ...
            {playerStats?.seasons?.[0] && (() => {
              ...
              const effUsg     = projOverrides.usg_pct ?? baseUsg
            ...
            {playerStats?.seasons?.length > 0 && (() => {
              const seasons = [...playerStats.seasons].reverse()
